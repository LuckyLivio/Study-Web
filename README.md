...
代码入口：[`client/src/App.tsx`](client/src/App.tsx)、[`server/index.js`](server/index.js)。可讲的技术点是前后端路由分工、MongoDB 模型和文件上传流程，以及将模拟功能逐步替换为真实服务时的接口边界。仓库未记录个人与团队职责分工，具体个人贡献需作者确认。
...
早期提交的前端构建产物包含 API 凭据；当前分支已将构建产物移出版本控制并忽略 `client/build/`。旧提交和缓存中的副本仍可能存在，相关凭据必须撤销或轮换。
